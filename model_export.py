...
def exportVertexCoords(content, pointer, vertex_amount, vertex_type, size, f_write):
    for i in range(pointer, pointer + 16 * vertex_amount, 16):
        ...
        f_write.write('v ' + str(format(x_vertex,'.6f')) + ' ' + str(format(y_vertex,'.6f')) + ' ' + str(format(z_vertex,'.6f')) + '\n')
        
def exportVertexCoordsOffset(content, pointer, vertex_amount, vertex_type, size, f_write):
    for i in range(pointer, pointer + 16 * vertex_amount, 16):
        
        x_vertex = convert_to_float32(convert_hex_to_binary32(convert_to_big_endian(content[i:i+4].hex()))) / 10 + 0.000001
        y_vertex = convert_to_float32(convert_hex_to_binary32(convert_to_big_endian(content[i+4:i+8].hex()))) / 10 + 0.000001
        z_vertex = convert_to_float32(convert_hex_to_binary32(convert_to_big_endian(content[i+8:i+12].hex()))) / 10 + 0.000001
...
def exportTextures(content, pointer, texture_amount, mtl_write, written_textures):
    textures = []
    for i in range(pointer, pointer + 144 * texture_amount, 144):
        ...
        if texture not in written_textures:
            ...
            mtl_write.write('map_Kd ..\\texture\\' + texture + '.png\n\n')
            written_textures.append(texture)
        textures.append(texture)
    return textures

def exportObjectFormations(content, pointer, objects_to_form, extra_vertex_count, extra_texture_maps_count, f_write, current_texture, textures, number_of_vertex_coords):
    ...
    while size_of_sub_object != 0:
        ...
        for i in range(size_of_sub_object):
            ...
            object_vertexes_offset.append(vertex + 1 + number_of_vertex_coords + extra_vertex_count)
        ...
        while len(object_vertexes) != 3:
            ...
            final_line = 'f '
            final_line += str(object_vertexes[2] + number_of_vertex_coords) + '/' + str(object_texture_maps[2]) + ' '
            final_line += str(object_vertexes[0] + number_of_vertex_coords) + '/' + str(object_texture_maps[0]) + ' '
            final_line += str(object_vertexes[1] + number_of_vertex_coords) + '/' + str(object_texture_maps[1]) + '\n'
            f_write.write(final_line)
...
def main(f):
    ...
    try:
        ...
        while objectListEntryPointer != 0:
            ...
            exportVertexCoordsOffset(content, objects['ToVertexCoords'], objects['NumberofVertexCoords'], 'v', 3, f_write)
            ...
            textures = exportTextures(content, objects['ToTextureList'], objects['NumberofTextureEntries'], mtl_write, written_textures)
            ...
            exportObjectFormations(content, objects['ToObjectFormationList'], objects['NumberofObjectsToForm'], extra_vertex_count, extra_texture_maps_count, f_write, current_texture, textures, objects['NumberofVertexCoords'])
            extra_vertex_count += objects['NumberofVertexCoords'] * 2
